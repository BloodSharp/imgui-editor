...
void homepageSystem::Render()
{
	...
	ImGui::Begin("Homepage", 0, flags);
	{
		...
		if (elements::Widget("Telegram Channel", "Application development blog.", (const char*)ICON_FA_PAPER_PLANE, { 270, 50 })) 
		{
		...
		if (elements::Widget("New design file", "Design and prototype", (const char*)ICON_FA_FILE_PLUS, { 300, 50 })) {
			gModalManager->Run([]() { 
				...
				ImGui::InputText("Project name", name, 64);
				ImGui::Spacing();
				ImGui::PushItemWidth(196);
				ImGui::InputFloat("Window Size X", &gProjectSystem->data.window.WindowSize.x, 0, 0, "%0.f");
				ImGui::SameLine();
				ImGui::PushItemWidth(196);
				ImGui::InputFloat("Window Size Y", &gProjectSystem->data.window.WindowSize.y, 0, 0, "%0.f");
				ImGui::Spacing();
				if (ImGui::Button("Create", ImVec2(380, 35))) {
